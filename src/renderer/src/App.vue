    ...
    <div class="app-layout">
      <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
          <button class="nav-item" :class="{ active: currentView === 'downloads' }" @click="currentView = 'downloads'">
            <i class="pi pi-download"></i>
            <span>{{ t('nav.downloads') }}</span>
            <Tag v-if="activeCount > 0" :value="String(activeCount)" severity="info" rounded style="margin-left:auto;font-size:10px" />
          </button>
            ...
            <div v-else class="download-list">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
                ...
                <div style="display:flex;gap:6px">
                  <Button v-if="pendingCount > 0" :label="t('downloads.start_all')" icon="pi pi-play" severity="success" size="small" outlined @click="startAllPending" />
                  <Button v-if="completedCount > 0" :label="t('downloads.clear_completed')" icon="pi pi-eraser" severity="secondary" size="small" text @click="clearCompleted" />
                </div>
              </div>
 ...
 * Componente raiz.
 *
 * Pré-carrega título+formatos via listFormats ao adicionar URLs.
...
import { useDownloads, DownloadStatus, type DownloadItem, type DownloadStage } from './stores/downloads';
...
const activeCount = computed(() => downloads.filter((d) => d.status === DownloadStatus.Downloading).length);
const completedCount = computed(() => downloads.filter((d) => d.status === DownloadStatus.Complete).length);
const pendingCount = computed(() => downloads.filter((d) => d.status === DownloadStatus.Pending).length);
