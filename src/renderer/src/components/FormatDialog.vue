<template>
  ...
  >
    <div v-if="loading" style="text-align: center; padding: 32px">
      ...
      <p
        v-if="timeoutWarning"
        style="margin-top: 8px; color: var(--warning-color); font-size: 12px"
      >
        {{ timeoutWarning }}
      </p>
    </div>
    ...
    <div v-else>
      ...
      <div v-if="selectedFormat" class="selected-summary">
        ...
        <strong>{{
          t('format.selected_summary', {
            resolution: getQualityLabel(selectedFormat.height, selectedFormat.resolution),
            vcodec: formatCodec(selectedFormat.vcodec),
      ...
      <div class="format-table-wrapper">
        ...
        >
          <Column field="format_id" :header="t('format.col.id')" style="width: 70px">
            <template #body="{ data }"
              ><span class="cell-text">{{ data.format_id }}</span></template
            >
          </Column>
          <Column field="ext" :header="t('format.col.ext')" style="width: 55px">
            <template #body="{ data }"
              ><span class="cell-text">{{ data.ext }}</span></template
            >
          </Column>
          ...
          <Column :header="t('format.col.fps')" style="width: 50px">
            <template #body="{ data }"
              ><span class="cell-text">{{ data.fps || '—' }}</span></template
            >
          </Column>
          ...
          <Column :header="t('format.col.type')" style="width: 85px">
            <template #body="{ data }"
              ><span :class="getTypeClass(data)">{{ getTypeLabel(data) }}</span></template
            >
          </Column>
          <Column field="format_note" :header="t('format.col.note')" style="min-width: 70px">
            <template #body="{ data }"
              ><span class="cell-text">{{ data.format_note }}</span></template
            >
          </Column>
        </DataTable>
      </div>
...
import { resolveRequestedFormatSelection, type SelectableFormat } from '../utils/format-selection'

const { t } = useI18n()
const { downloads, settings, getCachedFormats, ensureFormats } = useDownloads()
...
function getQualityLabel(height: number | null, resolution: string): string {
  if (!height) {
    return resolution || t('format.audio_only_short')
  }
...
function formatCodec(codec: string): string {
  ...
  if (codec.startsWith('vp08') || codec.startsWith('vp8')) {
    return 'VP8'
  }
...
function getTypeLabel(format: FormatInfo): string {
  ...
  if (hasVideo && hasAudio) {
    return t('format.type.both')
  }
  if (hasVideo) {
    return t('format.type.video')
  }
  return t('format.type.audio')
}
...
function preselectResolvedFormat(processedFormats: FormatInfo[]): void {
  const currentItem = downloads.find((item) => item.id === props.downloadId)
  const preferredFormatId = currentItem?.formatId || settings.defaultFormat
  const resolved = resolveRequestedFormatSelection(
    processedFormats as SelectableFormat[],
    preferredFormatId
  )

  selectedFormat.value = resolved?.selectedFormat || processedFormats[0] || null
}
...
async function loadFormats(): Promise<void> {
  ...
  try {
    ...
    if (cached) {
      ...
      videoDuration.value = cached.duration || 0
      formats.value = processFormats(cached.formats as FormatInfo[])
      preselectResolvedFormat(formats.value)
      return
    }
    ...
    // Aviso apenas visual; nao cancela a request principal.
    ...
    formats.value = processed
    preselectResolvedFormat(processed)
  } catch (err: unknown) {
    if (requestId !== activeRequestId) {
      return
    }
